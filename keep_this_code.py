...
for i in range(len(df)) :
    if macd[i] < 0 and macdsignal[i] < 0 and macd[i] > macdsignal[i] and macd[i-1] < macdsignal[i-1] and df['close'][i+1] > ema_200[i+1]:
        ...
        target_price = purchase_price * profit_ratio
    ...
    # Hold the Bitcoin until the MACD line crosses signal line above zero line
        for j in range(i, len(df)):
            if macd[j] > macdsignal[j] and macd[j-1] < macdsignal[j-1] and macd[j] > 0 or purchase_price>=target_price:
            ...
            elif macd[j] < macdsignal[j] and macd[j-1] > macdsignal[j-1] and macd[j] < 0 or purchase_price<=stop_loss_price:
                sell_price = df['close'][j+1]
...
apds = [
    mpf.make_addplot(macd, panel=1, color='orange', ylabel='MACD', width=0.75, secondary_y=False),
    mpf.make_addplot(macdsignal, panel=1, color='blue', width=0.75, secondary_y=False),
...
fig.text(0.2, 0.58, f"Balance - {balance}$", fontsize=12, fontweight='bold', color='black')
fig.text(0.2, 0.54, f"Profit - {formatted_profit}$", fontsize=12, fontweight='bold', color='green')
fig.text(0.2, 0.50, f"Loss - {loss}$", fontsize=12, fontweight='bold', color='red')
