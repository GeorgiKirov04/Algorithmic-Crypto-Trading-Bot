...
timeframe = '5m'
since = exchange.milliseconds() - 1000 * 60 * 60 * int(timeframe[:-1]) * 300
...
for i in range(len(df)):
    ...
    if macd[i] < 0 and macdsignal[i] < 0 and macd[i] > macdsignal[i] and df['open'][i+1] > ema_50[i+1]: 
        ...
        if trend_up and not in_position:
            ...
            if btc_bought < max_buy_percentage:
